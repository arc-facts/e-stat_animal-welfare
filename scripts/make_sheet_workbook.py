...
【重要】このブックに claims タブは含まれない。主張の台帳はスプレッドシート上
だけで管理しており、リポジトリに元データを持たないため。既存のシートに対して
「ファイル → インポート → スプレッドシートを置換する」を実行すると
claims タブごと消え、ボランティアの記入内容が失われる。既存シートを更新する
場合は「新しいシートを挿入する」を選び、差し替えたいタブだけ手で入れ替えること。
...
GUIDE = [
    ...
    ("※ claims タブはこのシート上だけで管理しています（氏名が公開されないよう、", None),
    ("　 リポジトリには元データも同期先も置いていません）。", None),
    ("　 このタブを消すと復元できません。シートの一括置換は行わないでください。", None),
    ("", None),
    ("出典の2列について", "h2"),
